...
/**
 * タスクをシャットダウンするためのハンドラークラスです．
 * 
 * @author j5ik2o
 */
public class ScheduleExecuteShutdownProcessor extends
		    ...
		    new TaskScheduleEntryHanlder<Void>() {
			    public Void processTaskScheduleEntry(
				    ...
				    if (tes.getTaskPropertyReader().isShutdownTask(false)) {
					    log.log("DCHRONOS0011", new Object[] { tes
					        .getTaskPropertyReader()
					        .getTaskName(null) });
					    final Future<TaskExecutorService> future =
					        executorService
